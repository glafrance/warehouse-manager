...
public class SecurityConfig {
    @Bean
    @Order(0) // Higher priority than the main chain
    public SecurityFilterChain healthCheckFilterChain(HttpSecurity http) throws Exception {
        http
                .securityMatcher("/actuator/health", "/actuator/health/**", "/api/health")
                .authorizeHttpRequests(auth -> auth.anyRequest().permitAll())
                // Disable everything for these specific paths
